...
// EmployeeAuthCircuit defines the zk-SNARK proof structure
// This proves an employee has access without revealing identity

type EmployeeAuthCircuit struct {
...
func (c *EmployeeAuthCircuit) Define(api frontend.API) error {
...
func GenerateProof(artifact Artifact, pk groth16.ProvingKey) ([]byte, error) {
	...
	var circuit EmployeeAuthCircuit
	...
	// Create witness
	witness, err := frontend.NewWitness(&EmployeeAuthCircuit{
...
func VerifyProof(artifact Artifact, proofBytes []byte, vk groth16.VerifyingKey) bool {
	...
	modulus := fr.Modulus()
	publicWitness, err := frontend.NewWitness(&EmployeeAuthCircuit{
		MerkleRoot: merkleRoot,
