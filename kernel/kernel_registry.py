...
def initialize_kernels() -> KernelRegistry:
    ...
    if mps_available:
        ...
        try:
            from caramba.kernel.metal.jit import load_caramba_metal_ops

            _ = load_caramba_metal_ops(verbose=False)
            from caramba.kernel.metal.attention_jit import load_caramba_metal_attention_ops

            _ = load_caramba_metal_attention_ops(verbose=False)
            from caramba.kernel.metal.resonant_jit import load_caramba_metal_resonant_ops
    ...
    if cuda_available:
        ...
        from caramba.kernel.kernels_decoupled import (
            kv_decode_partition_stats_decoupled_q4q8q4,
            kv_decode_reduce_partitions,
            kv_decode_update_decoupled_q4q8q4,
        )
        from caramba.kernel.triton.flash_attention_triton_kernels_bwd import (
            flash_attn_bwd_dkv,
            flash_attn_bwd_dq,
            flash_attn_bwd_preprocess,
        )
        from caramba.kernel.triton.flash_attention_triton_kernels_fwd import flash_attn_fwd
        from caramba.kernel.triton.dba_attention_triton_kernels_bwd import (
            dba_attn_bwd_dkv,
            dba_attn_bwd_dq,
            dba_attn_bwd_preprocess,
        )
        from caramba.kernel.triton.dba_attention_triton_kernels_fwd import dba_attn_fwd
        from caramba.kernel.kernels_ssm import selective_scan_triton
        from caramba.kernel.triton.rmsnorm_triton_kernels import rmsnorm_fwd, rmsnorm_bwd_x, rmsnorm_bwd_x_noweight, rmsnorm_bwd_w
        from caramba.kernel.triton.layernorm_triton_kernels import layernorm_fwd, layernorm_bwd_x, layernorm_gradw, layernorm_gradb
        from caramba.kernel.triton.rope_triton_kernels import rope_fwd, rope_bwd
