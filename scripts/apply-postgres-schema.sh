...
docker compose exec postgres psql -U postgres -d electric -v ON_ERROR_STOP=1 \
	-f /docker-entrypoint-initdb.d/09-paper-blocks.sql

echo "Schema applied on database 'electric' (research_projects, kanban, research_papers, project_members, researcher_profiles, paper_blocks)."
