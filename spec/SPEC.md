...
- **Low-level compute:** `pkg/backend/device/cpu` — 30 operation domains (`activation` … `vsa`, excluding shared `cpu/neon`) with Go scalar on all; **NEON** registered in 20/30 domains; **amd64 AVX-512** in `activation`, `attention`, `causal`, `convolution`, `dequant`, `dot`, `dropout`, `elementwise`, `embedding`, `layernorm`, `losses`, `masking`, `math`, `matmul`, `normalization`, `optimizer`, `pool`, `pospop`, `quant`, `reduction`, `rope`, `sampling`, and `shape` (23/30); **AVX2/SSE2** only in `activation` and `pospop`. Legacy **386** paths removed; CPU targets **amd64** and **arm64** only (`GOARCH=386` unsupported).
...
- **AVX-512** incomplete across CPU domains (registered on `activation`, `attention`, `causal`, `convolution`, `dequant`, `dot`, `dropout`, `elementwise`, `embedding`, `layernorm`, `losses`, `masking`, `math`, `matmul`, `normalization`, `optimizer`, `pool`, `pospop`, `quant`, `reduction`, `rope`, `sampling`, `shape` — 23/30 per dispatch matrix; **T2.1/T2.2** gap-close await human §10b only); `dispatchaudit` `TestValidateCPUDispatchMatrix` still fails 30 vs 32 domain invariant (pre-existing); **backend legality** and **zero-copy residency** tests not implemented.
