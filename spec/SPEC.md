...
- **Device backends:** `pkg/backend/device/{metal,cuda,xla}` — **Metal** has **494** `kernels.Default` registrations in WT (**~74/119** required ops registered per live `backendaudit` after `convolution.conv_transpose2d` audit cross-link; committed `docs/device-backend-matrix.md` still snapshots **462** / **68** — regen non-blocking); **CUDA** and **XLA** are tensor upload/download only (0 kernel registrations). **`device.Backend` ↔ `ir.RequiredOperationIDs()` inventory** in `docs/backend-inventory.md` and `pkg/backend/device/inventory*.go` (151 methods, 119 required ops cross-linked). **CPU dispatch matrix** in `docs/cpu-dispatch-matrix.md` and `pkg/backend/device/cpu/dispatchaudit/`. **Device backend matrix** in `docs/device-backend-matrix.md` and `pkg/backend/device/backendaudit/`. **Combined backend coverage** in `docs/backend-coverage.md` and `pkg/backend/device/coverageaudit/` (T1.2–T1.4 registration snapshot, R1 execution-target summary).
...
### Developer
- **Default next task: T3.1 slice 11** — Metal **`shape.slice`**: register `kernels.Default` **`"slice"`** for F32/F16/BF16; Required op **`shape.slice`** (`pkg/backend/compute/ir/required_operations.go:60`); registry cross-link `pkg/backend/device/cpu/shape` (`inventory_links.go:71`). Extend `pkg/backend/device/metal/shape.go` (no `"slice"` registration today) + Metal kernel + darwin dispatch; regenerate `kernels.metallib`.
- **Scalar reference:** CPU shape contiguous slice/copy (`shape_extended.go:17` documents slice); add or wire `runSlice` scalar in `cpu/shape` if absent — **0 ULP** parity at `parityElementCounts` for pure copy (acceptance §11).
- **Gated GLU + gelu/swiglu slices 2–10 done (WT, cycles 61–69):** do **not** reopen unless regression FAIL. Live `backendaudit`: `OpGELU`, `OpSwiGLU`, `activation.gelu`, `activation.swiglu` already **registered** on Metal.
- **Metal parity reference (binding):** `*_expected_test.go` calls **CPU domain generic**, not duplicated device math in Go tests.
- **T2.1/T2.2:** §10b human-only on arm64. **Do not** reopen **T2.3–T2.30**.

### Reviewer
- On **arm64 Curious host:** `5_verification` PASS when deliverables are in the **working tree**, host-runnable tests pass; **do not** FAIL for uncommitted WT or missing amd64+AVX512F paste (§10b human-only for T2.1/T2.2).
- Fail `3_spec_compliance` for duplicated kernel math in tests, widened ULP, missing `parityElementCounts`, unmasked tails, scalar-in-SIMD, or host-side masquerading — judge **working tree**, not `HEAD` alone.
- **T3.1 slice rule:** PASS only slice 11; roadmap **T3.1** stays `[ ]` until full Metal `device.Backend`.
- Regression: slice 11 runs include gated-activation + `gelu`/steer/graft tests (cycles 64–69 pattern).
- **Do not** FAIL slice 11 because `docs/device-backend-matrix.md` still shows `GELU`/`SwiGLU` as `—`; trust live `backendaudit` (registered in WT).
...
- **T3.1:** document slice PASS/FAIL in Progress; roadmap **T3.1** `[ ]` until full Backend on Metal.
