...
- **Low-level compute:** `pkg/backend/device/cpu` — 30 operation domains (`activation` … `vsa`, excluding shared `cpu/neon`) with Go scalar on all; **NEON** registered in 20/30 domains; **amd64 AVX-512** in `activation`, `attention`, `causal`, `convolution`, `dequant`, `dot`, `dropout`, `elementwise`, `embedding`, `hawkes`, `layernorm`, `losses`, `masking`, `math`, `matmul`, `normalization`, `optimizer`, `pool`, `pospop`, `quant`, `reduction`, `rope`, `sampling`, and `shape` (24/30); **AVX2/SSE2** only in `activation` and `pospop`. Legacy **386** paths removed; CPU targets **amd64** and **arm64** only (`GOARCH=386` unsupported).
...
- **AVX-512** incomplete across CPU domains (registered on `activation`, `attention`, `causal`, `convolution`, `dequant`, `dot`, `dropout`, `elementwise`, `embedding`, `hawkes`, `layernorm`, `losses`, `masking`, `math`, `matmul`, `normalization`, `optimizer`, `pool`, `pospop`, `quant`, `reduction`, `rope`, `sampling`, `shape` — 24/30 per dispatch matrix; **T2.1/T2.2** gap-close await human §10b only); `dispatchaudit` `TestValidateCPUDispatchMatrix` still fails 30 vs 32 domain invariant (pre-existing); **backend legality** and **zero-copy residency** tests not implemented.
...
### Developer
- **Default next task: T2.21** (`optimizer` AVX-512 greenfield — same contract as T2.3–T2.20: `f32_avx512_amd64.{s,go}`, new `select_amd64.go`, `select_other.go` `!amd64`, `optimizer_avx512_{parity,compliance,bench}_test.go`, `dispatchaudit` + `docs/cpu-dispatch-matrix.md` → **22/30**). amd64 today uses scalar via `select_other.go` (`adamStepSlices` → `adamStepSlicesScalar`); **mirror NEON** step symbols in `f32_neon_arm64.{s,go}` (`AdamStepFloat32NEONAsm`, `SgdStepFloat32NEONAsm`, `AdamwStepFloat32NEONAsm`, …). Domain is multi-kernel — one `.s` per ISA file, vectorized w16/w8/w4 + masked tails on each `*AVX512*` entry.
- **Do not** reopen verification-only **T2.1** / **T2.2** except human §10b (native amd64+AVX512F paste or CI URL). **T2.20** landed (`quant/`, `dequant/` on `08122c8`, docs **21/30**); do not re-develop quant/dequant unless review FAIL on new evidence.
