...
- [x] T2.17 — `shape` AVX-512: greenfield `f32_avx512_amd64.{s,go}` (`CopyContiguousFloat32AVX512Asm`, `WhereFloat32AVX512Asm`, `MaskedFillFloat32AVX512Asm`; masked tails `f32_avx512_amd64.s:49-60,:133-159,:221-245`; mask bit-offset `R11`); `f32_generic.go`, `f32_native.go`, `select_amd64.go`, `select_other.go`; `shape_ops.go` wired natives; `shape_f32_parity_test.go` + `shape_avx512_{parity,compliance,bench}_test.go` at `parity.Lengths` (0 ULP); cycle 39 review PASS; `GOARCH=amd64 go build ./pkg/backend/device/cpu/shape/` exit 0; arm64 `go test ./pkg/backend/device/cpu/shape/...` + `complianceaudit` pass; docs/dispatchaudit **17/30** (WT uncommitted); optional human amd64+AVX512F: `Test*F32AVX512Parity` / `Benchmark*F32AVX512*` (requirement: R1, R2)
