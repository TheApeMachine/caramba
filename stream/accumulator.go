...
func (accumulator *Accumulator) Compile() provider.Event {
	...
	if accumulator.err != nil {
		errEvent := provider.NewEvent("generate:error", provider.EventError, accumulator.err.Error(), "", nil)
		return *errEvent
	}

	out := provider.NewEvent("generate:stop", provider.EventStop, "\n", "", nil)
