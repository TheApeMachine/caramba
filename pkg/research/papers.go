...
/*
PaperService persists paper documents for research projects, with a
...
func NewPaperService(databaseURL string) *PaperService {
	return &PaperService{pool: apidata.NewSQLPool(databaseURL)}
}

func (service *PaperService) Create(ctx fiber.Ctx) error {
	return apidata.Mutate(ctx, "research paper", service.create)
}

func (service *PaperService) create(
	ctx fiber.Ctx,
	...
	if orgSlug == "" {
		return 0, apidata.Forbidden(errors.New("organization scope required for paper create"))
	}
...
func (service *PaperService) Update(ctx fiber.Ctx) error {
	...
	txid, updateErr := apidata.RunWithTxid(ctx, database, func(transaction *sql.Tx) error {
		now := time.Now().UTC()
...
func (service *PaperService) requireProjectInOrganization(
	transaction *sql.Tx,
	ctx fiber.Ctx,
	projectID string,
	orgSlug string,
	...
	if strings.TrimSpace(rowOrg) != orgSlug {
		return apidata.Forbidden(errors.New("project is not in this organization"))
	}

	return nil
}

func (service *PaperService) queryCurrentRevision(
	transaction *sql.Tx,
	ctx fiber.Ctx,
	paperID string,
	orgSlug string,
...
func (service *PaperService) defaultDocumentJSON() json.RawMessage {
