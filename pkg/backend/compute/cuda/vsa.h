...
// the opposite direction and all shifts are normalized modulo n. shift==0
...
// Inverse cyclic permutation by shift positions. This restores the original
// array when called with the same shift after cuda_vsa_permute:
// cuda_vsa_inverse_permute(cuda_vsa_permute(v,s), s) == v. Positive shift uses
// the same direction definition as cuda_vsa_permute and is converted to the
// opposite normalized rotation internally without overflowing INT_MIN. n must be
// > 0, |shift| may exceed n, and invalid arguments return -1. src and out must
// not alias.
