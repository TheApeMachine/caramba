...
func (tensorBackend *TensorBackend) applyModelOperation(
	...
	default:
		return nil, fmt.Errorf(
			"cuda tensor: operation %q node %q has no resident CUDA implementation",
			node.Op,
			node.ID,
		)
	}
...
func (tensorBackend *TensorBackend) applyReshape(
	...
	if len(inputs) != 1 {
		return nil, fmt.Errorf("cuda tensor: reshape node %q requires 1 input", node.ID)
	}
	...
	return tensorBackend.CopyTensor(inputs[0], outputShape)
}
...
func (tensorBackend *TensorBackend) applyTranspose(
	...
	if len(inputs) != 1 {
		return nil, fmt.Errorf("cuda tensor: transpose node %q requires 1 input", node.ID)
	}
...
func (tensorBackend *TensorBackend) applyConcat(
	...
	if len(inputs) < 2 {
		return nil, fmt.Errorf("cuda tensor: concat node %q requires at least 2 inputs", node.ID)
	}
...
func (tensorBackend *TensorBackend) applySplit(
	...
	if len(inputs) != 1 {
		return nil, fmt.Errorf("cuda tensor: split node %q requires 1 input", node.ID)
	}
	...
	if dimension < 0 || dimension >= len(inputShape) {
		return nil, fmt.Errorf("cuda tensor: split node %q dimension out of range", node.ID)
	}
...
func (tensorBackend *TensorBackend) applyUpsampleNearest2D(
	...
	if len(inputs) != 1 {
		return nil, fmt.Errorf("cuda tensor: upsample_nearest2d node %q requires 1 input", node.ID)
	}

	inputShape := inputs[0].Shape().Dims()

	if len(inputShape) != 4 {
		return nil, fmt.Errorf("cuda tensor: upsample_nearest2d node %q expects rank 4", node.ID)
	}
	...
	if len(outputDims) != 4 {
		return nil, fmt.Errorf("cuda tensor: upsample_nearest2d node %q output must be rank 4", node.ID)
	}
...
func (tensorBackend *TensorBackend) applyViewAsHeads(
	...
	if len(inputs) != 1 {
		return nil, fmt.Errorf("cuda tensor: view_as_heads node %q requires 1 input", node.ID)
	}

	inputShape := inputs[0].Shape().Dims()

	if len(inputShape) != 3 {
		return nil, fmt.Errorf("cuda tensor: view_as_heads node %q expects rank 3", node.ID)
	}

	numHeads := intConfig(node, "num_heads", 0)

	if numHeads <= 0 || inputShape[2]%numHeads != 0 {
		return nil, fmt.Errorf("cuda tensor: invalid head count %d", numHeads)
	}
...
func (tensorBackend *TensorBackend) applyMergeHeads(
	...
	if len(inputs) != 1 {
		return nil, fmt.Errorf("cuda tensor: merge_heads node %q requires 1 input", node.ID)
	}

	inputShape := inputs[0].Shape().Dims()

	if len(inputShape) != 4 {
		return nil, fmt.Errorf("cuda tensor: merge_heads node %q expects rank 4", node.ID)
	}
...
func (tensorBackend *TensorBackend) applyLastToken(
	...
	if len(inputs) != 1 {
		return nil, fmt.Errorf("cuda tensor: last_token node %q requires 1 input", node.ID)
	}

	inputShape := inputs[0].Shape().Dims()

	if len(inputShape) < 2 {
		return nil, fmt.Errorf("cuda tensor: last_token node %q expects rank >= 2", node.ID)
	}
...
func splitOuterInner(shape []int, dimension int) (int, int, error) {
	...
	for _, value := range shape[:dimension] {
		if value <= 0 {
			return 0, 0, fmt.Errorf("cuda tensor: split outer dimensions must be positive")
		}
	...
	for _, value := range shape[dimension+1:] {
		if value <= 0 {
			return 0, 0, fmt.Errorf("cuda tensor: split inner dimensions must be positive")
		}
...
func lastTokenOuterLength(shape []int) (int, error) {
	...
	for _, value := range shape[:len(shape)-2] {
		if value <= 0 {
			return 0, fmt.Errorf("cuda tensor: last_token outer dimensions must be positive")
		}
