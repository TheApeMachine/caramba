...
func (gqa *GQA) Forward(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireOperationInputs("cuda.attention.gqa", 3); err != nil {
		return nil, err
	}
...
func (rope *RoPE) Forward(stateDict *state.Dict) (*state.Dict, error) {
	...
	output, err := rope.positional.RoPEForwardAtModeConfig(
		rotary.Config{
			Base:                          defaultCUDAFloat(stateDict.Base, 10000),
...
func (convolution *Conv2D) Forward(stateDict *state.Dict) (*state.Dict, error) {
	...
	output, err := convolution.convolution.Conv2d(
		...
		defaultCUDAInt(defaultCUDAInt(stateDict.StrideH, stateDict.Stride), 1),
		defaultCUDAInt(defaultCUDAInt(stateDict.StrideW, stateDict.Stride), 1),
		stateDict.PadH, stateDict.PadW,
		defaultCUDAInt(defaultCUDAInt(stateDict.DilationH, stateDict.Dilation), 1),
		defaultCUDAInt(defaultCUDAInt(stateDict.DilationW, stateDict.Dilation), 1),
...
func (convolution *Conv3D) Forward(stateDict *state.Dict) (*state.Dict, error) {
	...
	output, err := convolution.convolution.Conv3d(
		...
		defaultCUDAInt(stateDict.KernelH, stateDict.KernelSize),
		defaultCUDAInt(stateDict.KernelW, stateDict.KernelSize),
		defaultCUDAInt(defaultCUDAInt(stateDict.StrideD, stateDict.Stride), 1),
		defaultCUDAInt(defaultCUDAInt(stateDict.StrideH, stateDict.Stride), 1),
		defaultCUDAInt(defaultCUDAInt(stateDict.StrideW, stateDict.Stride), 1),
		stateDict.PadD, stateDict.PadH, stateDict.PadW,
		defaultCUDAInt(defaultCUDAInt(stateDict.DilationD, stateDict.Dilation), 1),
		defaultCUDAInt(defaultCUDAInt(stateDict.DilationH, stateDict.Dilation), 1),
		defaultCUDAInt(defaultCUDAInt(stateDict.DilationW, stateDict.Dilation), 1),
