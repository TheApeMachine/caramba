...
func (adam *Adam) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda adam"); err != nil {
		return nil, err
	}

	stateDict.Step++
	stateDict.EnsureOut()
	learningRate := biasCorrectedLearningRate(stateDict)
	rc := C.cuda_optimizer_adam(
	...
	return stateDict, cudaOptimizerError(rc, "adam")
}

type AdamW struct{}

func (adamW *AdamW) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda adamw"); err != nil {
		return nil, err
	}

	stateDict.Step++
	stateDict.EnsureOut()
	learningRate := adamwBiasCorrectedLearningRate(stateDict)
	rc := C.cuda_optimizer_adamw(
		ptr(stateDict.Out), ptr(stateDict.M), ptr(stateDict.V),
		ptr(stateDict.Params), ptr(stateDict.Grads), C.int(len(stateDict.Params)),
		C.double(stateDict.Beta1), C.double(stateDict.Beta2),
		C.double(learningRate), C.double(stateDict.Eps),
	...
	return stateDict, cudaOptimizerError(rc, "adamw")
}

type AdaMax struct{}

func (adaMax *AdaMax) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda adamax"); err != nil {
		return nil, err
	}
	...
	return stateDict, cudaOptimizerError(rc, "adamax")
}

type SGD struct{}

func (sgd *SGD) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda sgd"); err != nil {
		return nil, err
	}

	stateDict.EnsureOut()
	rc := C.cuda_optimizer_sgd(
...
func (rmsProp *RMSProp) Step(stateDict *state.Dict) (*state.Dict, error) {
	...
	return stateDict, cudaOptimizerError(rc, "rmsprop")
}

type Hebbian struct{}

func (hebbian *Hebbian) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda hebbian"); err != nil {
		return nil, err
	}

	stateDict.EnsureOut()
	rc := C.cuda_optimizer_hebbian(
	...
	return stateDict, cudaOptimizerError(rc, "hebbian")
}

type Lars struct{}

func (lars *Lars) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda lars"); err != nil {
		return nil, err
	}

	stateDict.EnsureOut()
	rc := C.cuda_optimizer_lars(
		ptr(stateDict.Out), ptr(stateDict.M), ptr(stateDict.Params), ptr(stateDict.Grads),
	...
	return stateDict, cudaOptimizerError(rc, "lars")
}

type Lamb struct{}

func (lamb *Lamb) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda lamb"); err != nil {
		return nil, err
	}
	...
	return stateDict, cudaOptimizerError(rc, "lamb")
}

type AdaGrad struct{}

func (adaGrad *AdaGrad) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda adagrad"); err != nil {
		return nil, err
	}
	...
	return stateDict, cudaOptimizerError(rc, "adagrad")
}

type AdaDelta struct{}

func (adaDelta *AdaDelta) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda adadelta"); err != nil {
		return nil, err
	}

	stateDict.EnsureOut()
	stateDict.EnsureBuf()
	rc := C.cuda_optimizer_adadelta(
		ptr(stateDict.Out), ptr(stateDict.V), ptr(stateDict.Buf),
		ptr(stateDict.Params), ptr(stateDict.Grads), C.int(len(stateDict.Params)),
		C.double(stateDict.Rho), C.double(stateDict.Eps), C.double(stateDict.WD),
	)

	return stateDict, cudaOptimizerError(rc, "adadelta")
}

type LBFGS struct{}

func (lbfgs *LBFGS) Step(stateDict *state.Dict) (*state.Dict, error) {
	if err := stateDict.RequireReady("cuda lbfgs"); err != nil {
		return nil, err
	}

	stateDict.EnsureOut()
	stateDict.EnsureHistory()
	previousParams, previousGrads := lbfgsPrevious(stateDict)
	sHistory := flattenHistory(stateDict.SHist, stateDict.HistSize, len(stateDict.Params))
	yHistory := flattenHistory(stateDict.YHist, stateDict.HistSize, len(stateDict.Params))
	head := C.int(stateDict.Head)
	count := C.int(stateDict.Count)
	rc := C.cuda_optimizer_lbfgs(
	...
	if err := cudaOptimizerError(rc, "lbfgs"); err != nil {
		return nil, err
	}

	stateDict.Head = int(head)
	stateDict.Count = int(count)
	expandHistory(stateDict.SHist, sHistory, len(stateDict.Params))
	expandHistory(stateDict.YHist, yHistory, len(stateDict.Params))
...
func biasCorrectedLearningRate(stateDict *state.Dict) float64 {
...
func adamwBiasCorrectedLearningRate(stateDict *state.Dict) float64 {
	bc1 := 1 - stdmath.Pow(stateDict.Beta1, float64(stateDict.Step))
	bc2 := 1 - stdmath.Pow(stateDict.Beta2, float64(stateDict.Step))
...
func lbfgsPrevious(stateDict *state.Dict) ([]float64, []float64) {
...
func flattenHistory(history [][]float64, historySize, count int) []float64 {
...
func expandHistory(history [][]float64, flat []float64, count int) {
...
func cudaOptimizerError(rc C.int, name string) error {
	if rc == 0 {
		return nil
	}

	return fmt.Errorf("cuda optimizer: %s fused kernel failed", name)
}
