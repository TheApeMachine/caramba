...
func (op *CUDAMarkovBlanket) Partition(shape []int, data ...[]float64) ([]float64, error) {
	...
	if rc != 0 {
		return nil, fmt.Errorf("cuda_mb_partition failed (rc=%d)", rc)
	}
	return out, nil
}

// FlowInternal: out[Ni] = W[Ni*Ns] @ x_sens[Ns] + bias[Ni]
func (op *CUDAMarkovBlanket) FlowInternal(shape []int, data ...[]float64) ([]float64, error) {
	...
	if rc != 0 {
		return nil, fmt.Errorf("cuda_mb_flow_internal failed (rc=%d)", rc)
	}
	return out, nil
}

// FlowActive: out[Na] = W[Na*Ni] @ x_int[Ni] + bias[Na]
func (op *CUDAMarkovBlanket) FlowActive(shape []int, data ...[]float64) ([]float64, error) {
	...
	if rc != 0 {
		return nil, fmt.Errorf("cuda_mb_flow_active failed (rc=%d)", rc)
	}
	return out, nil
}

// MutualInformation: shape=[N,M], data[0]=X[T*N], data[1]=Y[T*M]. Returns scalar.
func (op *CUDAMarkovBlanket) MutualInformation(
	...
	if rc != 0 {
		return nil, fmt.Errorf("cuda_mb_mutual_information failed (rc=%d)", rc)
	}
	return out, nil
}
