...
// n is the total element count (num_samples * num_classes). logits and targets
// are row-major; sample i's class score j is at logits[i * num_classes + j].
// num_samples and num_classes must be > 0, and n must be divisible by
// num_classes. Probability targets should be in [0,1] and sum to 1 per sample;
// one-hot targets are accepted as the common probability-target special case.
...
// invalid arguments, allocation failure, or CUDA runtime failure. Reentrant
// across host threads with normal CUDA context constraints.
...
// Reentrant across host threads with normal CUDA context constraints.
...
// -1 on invalid arguments or CUDA failure. Reentrant across host threads with
// normal CUDA context constraints.
