...
// conv3dAVX2 — full conv3d forward pass using AVX2+FMA.
//
// func conv3dAVX2(out, x, weight, bias []float64,
//                 n, inC, d, h, w, outC,
...
c3_loop_ni:
	MOVQ c3NI(SP), AX; MOVQ c3N(SP), BX; CMPQ AX, BX; JGE c3_done
...
c3_loop_g:
	MOVQ c3GRP(SP), AX; MOVQ c3GROUPS(SP), BX; CMPQ AX, BX; JGE c3_next_ni
...
c3_loop_oc:
	MOVQ c3OC(SP), AX
	MOVQ c3OCSTART(SP), BX; MOVQ c3OCPERG(SP), CX; ADDQ BX, CX; CMPQ AX, CX; JGE c3_next_g

	// wRowP = weight + oc*icPerGroup*kD*kH*kW*8
...
c3_loop_do:
	MOVQ c3DO(SP), AX; MOVQ c3DOUT(SP), BX; CMPQ AX, BX; JGE c3_next_oc

	XORQ AX, AX; MOVQ AX, c3HO(SP)

c3_loop_ho:
	MOVQ c3HO(SP), AX; MOVQ c3HOUT(SP), BX; CMPQ AX, BX; JGE c3_next_do

	XORQ AX, AX; MOVQ AX, c3WO(SP)

c3_loop_wo:
	MOVQ c3WO(SP), AX; MOVQ c3WOUT(SP), BX; CMPQ AX, BX; JGE c3_next_ho
...
c3_loop_ic:
	MOVQ c3IC(SP), AX; MOVQ c3ICPERG(SP), BX; CMPQ AX, BX; JGE c3_write_out

	// absIC = icStart + ic; inputBase = (ni*inC + absIC)*d*h*w
...
c3_loop_kd:
	MOVQ c3KD_IDX(SP), AX; MOVQ c3KD(SP), BX; CMPQ AX, BX; JGE c3_next_ic

	// di = do*sD + kd*dilD - pD
	MOVQ c3DO(SP), BX; MOVQ c3SD(SP), CX; IMULQ CX, BX
	MOVQ c3KD_IDX(SP), CX; MOVQ c3DD(SP), DX; IMULQ DX, CX; ADDQ CX, BX
	MOVQ c3PD(SP), CX; SUBQ CX, BX  // BX = di
	MOVQ BX, c3DI(SP)

	CMPQ BX, $0; JL c3_next_kd
	MOVQ c3D(SP), CX; CMPQ BX, CX; JGE c3_next_kd

	XORQ AX, AX; MOVQ AX, c3KH_IDX(SP)

c3_loop_kh:
	MOVQ c3KH_IDX(SP), AX; MOVQ c3KH(SP), BX; CMPQ AX, BX; JGE c3_next_kd

	// hi = ho*sH + kh*dilH - pH
	MOVQ c3HO(SP), BX; MOVQ c3SH(SP), CX; IMULQ CX, BX
	MOVQ c3KH_IDX(SP), CX; MOVQ c3DH(SP), DX; IMULQ DX, CX; ADDQ CX, BX
	MOVQ c3PH(SP), CX; SUBQ CX, BX  // BX = hi
	MOVQ BX, c3HI(SP)

	CMPQ BX, $0; JL c3_next_kh
	MOVQ c3H(SP), CX; CMPQ BX, CX; JGE c3_next_kh

	// Check fast kW path: dilW==1, wo*sW >= pW, wo*sW+(kW-1) < w+pW
	MOVQ c3DW(SP), R8; CMPQ R8, $1; JNE c3_kw_scatter

	MOVQ c3WO(SP), R8; MOVQ c3SW(SP), R9; IMULQ R9, R8
	MOVQ c3PW(SP), R9; CMPQ R8, R9; JL c3_kw_scatter

	MOVQ R8, R10; SUBQ R9, R10; MOVQ c3KW(SP), R11; ADDQ R11, R10; SUBQ $1, R10
	MOVQ c3W(SP), R11; CMPQ R10, R11; JGE c3_kw_scatter

	// wi_start = wo*sW - pW
	MOVQ c3WO(SP), R10; MOVQ c3SW(SP), R11; IMULQ R11, R10; MOVQ c3PW(SP), R11; SUBQ R11, R10

	// xPtr = xP + (inputBase + di*h*w + hi*w + wi_start)*8
	...
	// wPtr = wRowP + (weightBase + kd*kH*kW + kh*kW)*8
	MOVQ c3WBASE(SP), R12
	...
	MOVQ c3KW(SP), R13
	MOVSD c3SUM(SP), X0; VXORPD Y1, Y1, Y1

c3_avx2_kw:
	CMPQ R13, $4; JL c3_avx2_kw_tail
	VMOVUPD 0(R11), Y2; VMOVUPD 0(R12), Y3; VFMADD231PD Y2, Y3, Y1
	ADDQ $32, R11; ADDQ $32, R12; SUBQ $4, R13; JMP c3_avx2_kw
c3_avx2_kw_tail:
	VEXTRACTF128 $1, Y1, X2; VADDPD X2, X1, X1; VHADDPD X1, X1, X1; ADDSD X1, X0
	CMPQ R13, $0; JLE c3_avx2_kw_done
c3_avx2_kw_sc:
	MOVSD (R11), X1; MOVSD (R12), X2; MULSD X2, X1; ADDSD X1, X0
	ADDQ $8, R11; ADDQ $8, R12; DECQ R13; JNZ c3_avx2_kw_sc
c3_avx2_kw_done:
	MOVSD X0, c3SUM(SP); VZEROUPPER; JMP c3_next_kh

c3_kw_scatter:
	XORQ R8, R8; MOVSD c3SUM(SP), X0
c3_kw_sc_loop:
	MOVQ c3KW(SP), R9; CMPQ R8, R9; JGE c3_kw_sc_done

	// wi = wo*sW + kw*dilW - pW
	MOVQ c3WO(SP), R9; MOVQ c3SW(SP), R10; IMULQ R10, R9
	MOVQ R8, R10; MOVQ c3DW(SP), R11; IMULQ R11, R10; ADDQ R10, R9
	MOVQ c3PW(SP), R10; SUBQ R10, R9  // R9 = wi

	CMPQ R9, $0; JL c3_kw_sc_next
	MOVQ c3W(SP), R10; CMPQ R9, R10; JGE c3_kw_sc_next

	// x[inputBase + di*h*w + hi*w + wi]
	...
	// weight[weightBase + kd*kH*kW + kh*kW + kw]
	MOVQ c3WBASE(SP), R10
...
c3_kw_sc_next:
	INCQ R8; JMP c3_kw_sc_loop
c3_kw_sc_done:
	MOVSD X0, c3SUM(SP)

c3_next_kh:
	MOVQ c3KH_IDX(SP), AX; INCQ AX; MOVQ AX, c3KH_IDX(SP); JMP c3_loop_kh

c3_next_kd:
	MOVQ c3KD_IDX(SP), AX; INCQ AX; MOVQ AX, c3KD_IDX(SP); JMP c3_loop_kd

c3_next_ic:
	MOVQ c3IC(SP), AX; INCQ AX; MOVQ AX, c3IC(SP); JMP c3_loop_ic

c3_write_out:
	// out[ni*outC*dOut*hOut*wOut + oc*dOut*hOut*wOut + do*hOut*wOut + ho*wOut + wo]
	...
	MOVQ c3WO(SP), AX; INCQ AX; MOVQ AX, c3WO(SP); JMP c3_loop_wo

c3_next_ho:
	MOVQ c3HO(SP), AX; INCQ AX; MOVQ AX, c3HO(SP); JMP c3_loop_ho

c3_next_do:
	MOVQ c3DO(SP), AX; INCQ AX; MOVQ AX, c3DO(SP); JMP c3_loop_do

c3_next_oc:
	MOVQ c3OC(SP), AX; INCQ AX; MOVQ AX, c3OC(SP); JMP c3_loop_oc

c3_next_g:
	MOVQ c3GRP(SP), AX; INCQ AX; MOVQ AX, c3GRP(SP); JMP c3_loop_g

c3_next_ni:
	MOVQ c3NI(SP), AX; INCQ AX; MOVQ AX, c3NI(SP); JMP c3_loop_ni
