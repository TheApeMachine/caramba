...
// tiedEmbeddingMatmulAVX2(dst, input, weight []float64, M, K, N int)
// ABI0 FP layout:
...
// logits[i,j] = sum_k hidden[i,k]*W[k,j], row-major for projection.tied_embedding.
TEXT ·tiedEmbeddingMatmulAVX2(SB), NOSPLIT, $0-96
	MOVQ dst+0(FP), R8
	MOVQ a+24(FP), R9
	MOVQ b+48(FP), R10
	MOVQ M+72(FP), R11   // M
