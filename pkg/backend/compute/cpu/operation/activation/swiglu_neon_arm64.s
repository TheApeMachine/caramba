...
// SwiGLUNEON(dst, gates, values []float64)
// Two-lane NEON swish(gate) * value.
TEXT ·SwiGLUNEON(SB), NOSPLIT, $0-72
	MOVD dst+0(FP), R0
	MOVD dst_len+8(FP), R2
	MOVD gates_base+24(FP), R1
	MOVD values_base+48(FP), R3
	LSR  $1, R2, R2
	CBZ  R2, swiglu_neon_done
