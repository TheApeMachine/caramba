...
func init() {
	// gather: (source paramDType, indices Int32) → paramDType
	for _, paramDType := range []dtype.DType{dtype.BFloat16, dtype.Float16} {
		...
		Default.Register(Kernel{
			...
			Run: func(args ...tensor.Tensor) error {
				return runShapeOpWithIntIndex(args, paramDType, runGatherFloat32Int32, 0)
			},
		})
		...
		Default.Register(Kernel{
			...
			Signature: Signature{
				Layout:  tensor.LayoutDense,
				Inputs:  []dtype.DType{paramDType, dtype.Int32, paramDType},
				Outputs: []dtype.DType{paramDType},
			},
			Locations: []tensor.Location{tensor.Host},
			Run: func(args ...tensor.Tensor) error {
				return runScatterMixed(args, paramDType)
			},
		})
		...
		Default.Register(Kernel{
			...
			Signature: Signature{
				Layout:  tensor.LayoutDense,
				Inputs:  []dtype.DType{paramDType},
				Outputs: []dtype.DType{paramDType},
			},
			Locations: []tensor.Location{tensor.Host},
			Run: func(args ...tensor.Tensor) error {
				return runShapeUnaryMixed(args, paramDType, runTranspose2DFloat32)
			},
		})
		...
		Default.Register(Kernel{
			...
			Locations: []tensor.Location{tensor.Host},
			Run: func(args ...tensor.Tensor) error {
				return runConcatMixed(args, paramDType)
			},
		})
	}
}
