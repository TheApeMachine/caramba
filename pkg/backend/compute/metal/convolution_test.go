...
func BenchmarkConvolutionOps_Conv2dTensor(benchmark *testing.B) {
	...
	inputShape := mustMetalConvolutionShape(benchmark, []int{1, 1, 1, 8192})
	weightShape := mustMetalConvolutionShape(benchmark, []int{1, 1, 1, 1})
	biasShape := mustMetalConvolutionShape(benchmark, []int{1})
	outputShape := mustMetalConvolutionShape(benchmark, []int{1, 1, 1, 8192})
	input := uploadMetalTensorForTest(
		benchmark,
		tensorBackend,
		inputShape,
...
func mustMetalConvolutionShape(test testing.TB, dims []int) computetensor.Shape {
	test.Helper()
