"""ARC-Challenge accuracy task

Loads and evaluates the ARC-Challenge dataset, which tests science question
answering with harder questions that require more domain knowledge.
"""
...
class BenchmarkAccuracyTaskArcChallenge(BenchmarkAccuracyTask):
    """ARC-Challenge accuracy task"""

    def iter_examples(self, *, split: str) -> Iterator[tuple[str, list[str], str, str]]:
        """Iterate over ARC-Challenge examples."""
        ds_arc: Any = load_dataset("ai2_arc", "ARC-Challenge", split=str(split))
        for ex in ds_arc:
