...
def main() -> None:
    ...
    # Bridge: semantic tokens -> spectral bins (one per token for demo simplicity).
    freqs = token_frequencies(len(vocab), device=device)
    bridge = BridgeManifold(num_semantic=len(vocab), num_spectral=len(vocab), dt=cfg.dt, device=device)

    # Co-activation training: present each token with its frequency as concurrent sensory evidence.
    sem_act = torch.zeros(len(vocab), device=device)
    spec_act = torch.zeros(len(vocab), device=device)
    for tid in range(len(vocab)):
        sem_act.zero_()
        spec_act.zero_()
        sem_act[tid] = 1.0
        spec_act[tid] = 1.0
        bridge.coactivate(torch.tensor([tid], device=device), sem_act, torch.tensor([tid], device=device), spec_act)
    ...
    # Bridge the semantic distribution into spectral energy.
    sem_dist = out.probs
    b_out = bridge.forward(sem_dist)
    spec_energy = b_out.freq_probs
