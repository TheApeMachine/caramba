...
class SparseBondGraph:
    ...
    def add_edges(self, src: torch.Tensor, dst: torch.Tensor, mass: torch.Tensor) -> None:
        ...
        # New edges have zero trace.
        trace_u = torch.zeros_like(mass_u)
        ...
        if exists.any():
            ...
            # trace_u is zeros; kept for completeness.
    ...
    def prune_by_src_mean(self, src_ids: torch.Tensor) -> None:
        """Prune weak edges using per-source mean threshold (no fixed constants).
        ...
        # Per-source mean over the active subset.
