...
class SemanticManifold(ThermodynamicEngine):
    ...
    def _active_sources_from_excitation(self) -> torch.Tensor:
        """Select active sources without fixed thresholds (locality derived from scale)."""
        exc = self.attractors.get("excitation").clamp(min=0.0)
        ids = torch.nonzero(exc > 0, as_tuple=False).flatten()
        if ids.numel() == 0:
            return ids
        # Derived locality: keep ~sqrt(nnz) most excited nodes.
        k = max(1, int(round(float(ids.numel()) ** 0.5)))
        k = min(k, int(ids.numel()))
        vals = exc[ids]
        top = torch.topk(vals, k=k).indices
        return ids[top]
    ...
    def observe_next_token(self, next_id: int, *, probs: Optional[torch.Tensor] = None, cur_id: Optional[int] = None) -> Dict[str, float]:
    ...
    def _ponder_dream_rollouts(self, active_src: torch.Tensor, *, ratio: torch.Tensor, steps: int) -> Dict[str, float]:
        ...
        # Rollout count derived from locality.
        rollouts = max(1, int(round(float(active_src.numel()) ** 0.5)))
        w = exc[active_src]
        w = w / (w.sum() + eps)
        ...
        for _ in range(rollouts):
            ...
            for _t in range(int(steps)):
                ...
                self.observe_next_token(nxt, probs=probs, cur_id=cur)
                cur = nxt
