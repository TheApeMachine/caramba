...
*   [x] **Implement Loss**: CrossEntropy loss between the new logits and `mosaic_teacher_commitment_delta` (mapping `{-1, 0, 1}` to class indices).
...
- Added `commitment_head_enabled: bool = False` to `MosaicBlockLayerConfig`
- When enabled, `MosaicBlockLayer` emits `mosaic_commitment_logits` with shape `(B, T, 3)` into `ctx.mosaic_aux_out`
...
- Dataset now emits `mosaic_teacher_commitment_delta` aligned to `input_ids`
