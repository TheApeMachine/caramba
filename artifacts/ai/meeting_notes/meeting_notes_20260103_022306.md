...
Add a commitment head:
 ...
 # In MemoryBlockLayerConfig
 commitment_head_enabled: bool = False

 # In MemoryBlockLayer.__init__
 if cfg.commitment_head_enabled:
     self.commitment_head = nn.Linear(cfg.d_model, 3)  # [close, neutral, open]

 # In forward(), emit to ctx.memblock_aux_out:
 if hasattr(self, 'commitment_head'):
     ctx.memblock_aux_out["mosaic_commitment_logits"] = self.commitment_head(x)  # (B, T, 3)
...
Output tensor:
 ...
 out["memblock_teacher_commitment_delta"] = torch.tensor(b.commitment_delta[:-1], dtype=torch.long)
...
───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
 ...
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  config/layer.py                        Add commitment_head_enabled: bool to MemoryBlockLayerConfig
...
───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
 ...
 1 Runtime: caramba run config/presets/mosaic_commitment.yml completes without errors
...
### Core Schema Updates
*   [x] **Modify `core/event.py`**: Update the `EventEnvelope` dataclass to include:
    *   `commitment_delta: int = 0` (Values: `-1` for close, `0` neutral, `+1` for open).
    *   `commitment_id: str | None = None` (UUID linking open/close pairs).

### Model & Configuration Updates
*   [x] **Update `config/layer.py`**: Add `commitment_head_enabled: bool = False`.
*   [x] **Update `layer/mosaic/block.py`**: Implement the **Commitment Head**:
    *   A linear projection (`d_model` → 3 logits) to predict the commitment state.
    *   Output stored in `ctx.memblock_aux_out["mosaic_commitment_logits"]`.

### Training Objectives
*   [x] **Update `trainer/objectives.py`**: Add `aux_commitment_weight`.
*   [x] **Implement Loss**: CrossEntropy loss between the new logits and `memblock_teacher_commitment_delta` (mapping `{-1, 0, 1}` to class indices).

### Dataset & Curriculum
*   [x] **Update `data/event_trace.py`**: Extend the builder to track commitments.
*   [x] **Create Negotiation Scenarios**: Generate synthetic traces where:
    1.  Agent accepts task ("I will...") → `delta=+1`
    2.  Agent works (Tools) → `delta=0`
    3.  Agent resolves ("Here is...") → `delta=-1`

### Execution
*   [x] **Create Preset**: `config/presets/mosaic_commitment.yml` (based on `mosaic_event_native` but with commitment heads enabled).
*   [x] **Validation (ready)**: Run `caramba run config/presets/mosaic_commitment.yml` and verify `aux_commitment_ce` loss trends downward.

---

## 4. Implementation Summary (Phase 2 — Completed)

### Commitment Meta-Fields (`core/event.py`)
- Added `commitment_delta: int = 0` with strict validation \(\{-1,0,1\}\)
- Added `commitment_id: str | None = None` (serialized only when present)

### Commitment Head (`config/layer.py`, `layer/mosaic/block.py`)
- Added `commitment_head_enabled: bool = False` to `MemoryBlockLayerConfig`
- When enabled, `MemoryBlockLayer` emits `mosaic_commitment_logits` with shape `(B, T, 3)` into `ctx.memblock_aux_out`

### Objective Support (`trainer/objectives.py`)
- Added `aux_commitment_weight`
- Added commitment CE loss and metrics (`aux_commitment_ce`, `aux_commitment_weighted`)
- Teacher masking uses `-100` (`ignore_index=-100`), while valid labels are `{-1,0,1}`

### Negotiation Curriculum (`data/event_trace.py`)
- Builder now tracks a per-token `commitment_delta` teacher stream
- Dataset now emits `memblock_teacher_commitment_delta` aligned to `input_ids`
- Added `negotiation_pairs` to generate open/neutral/close negotiation cycles using `commitment_id` linking

### Runnable Preset (`config/presets/mosaic_commitment.yml`)
- New manifest preset enabling the commitment head and training on negotiation traces

No remaining Phase 2 implementation items from this meeting (run the preset to confirm learning signals in your environment).