...
2. **Opcode assignment logic:**
   - `OP_WRITE` assigned to value tokens during "SET k IS v" phase (when `memblock_teacher_write_gate=1`)
...
3. **Output:**
   - Returns `memblock_teacher_opcode: Tensor` with shape `(T,)` and dtype `int64`
...
**Location:** `layer/mosaic/block.py` → `ctx.memblock_aux_out`
...
- Added `state_decay_reg_min` / `state_decay_reg_max` to `MemoryBlockLayerConfig` (`config/layer.py`)
- Each MOSAIC block now emits `mosaic_state_decay_reg_loss` (scalar) into `ctx.memblock_aux_out` and stacked blocks **accumulate** this scalar loss
...
- Teacher alignment includes `memblock_teacher_opcode`, routing buckets, and write gates at token spans within JSON literals
...
- [x] **Register traces**: synthetic datasets now supervise register write/selection when `reg_slots > 0`:
  - `data/mosaic_synth.py`: `memblock_teacher_reg_write_gate`, `memblock_teacher_reg_sel`
  - `data/event_trace.py`: `memblock_teacher_reg_write_gate`, `memblock_teacher_reg_sel`
