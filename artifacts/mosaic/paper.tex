...
The MOSAIC framing is intentionally modular: the associative cache is a \emph{bounded data structure} with a (router, scorer, updater) interface, not a monolithic mechanism. This makes it possible to (i) improve in-bucket selection with additional \emph{local} similarity signals, and (ii) attach \emph{control-plane} modules that shape future retrieval without changing constant-time lookup.
...
\]
This score is invariant to global phase rotation and robust to partial/noisy cues in phase space. When used \emph{within a bucket} of fixed associativity $A$ (or within RMF's top-$K$ candidate set), it remains $O(A)$ / $O(K)$ and preserves MOSAIC's constant-time contract.

\paragraph{Bounded cache vs.\ retrieval oracle.}
Our reference resonant associative implementation scores against all stored patterns (exact $O(KN)$) and is therefore not the production MOSAIC cache. Its role inside CCP is primarily:
(i) a \emph{diagnostic harness} for the addressing bottleneck (an explicit, auditable retrieval oracle for stress tests), and
(ii) a prototype substrate for RMF-style successor fields, where scoring is applied to a bounded candidate set (top-$K$) or a fixed-size set-associative table (router emits the address; local similarity selects among $A$ slots).
...
\begin{itemize}
    ...
    \item \textbf{Resonant Memory Field (RMF) substrate:} \path{resonant/core/associative\_memory/}\\Explicit phasor-coded associative recall and diagnostic harness.
\end{itemize}
