...
This paper is a \textit{manifest-first} report: all training runs are executed via the Caramba manifest runner (\texttt{python -m caramba.cli run}) using the paper presets in \path{config/presets/}. We are rerunning the primary baseline vs.\ DBA comparison in Caramba (switching from the earlier production codebase) and will update the reported step counts and figures as the new runs complete. Training uses FineWeb-Edu tokens and a lightweight benchmark script (\path{eval_ckpt.py}) for held-out evaluation. We also demonstrate long-context inference \textit{stability} up to \textbf{131{,}072 tokens} on consumer hardware, while noting that perplexity at extreme context lengths can degrade sharply due to RoPE extrapolation (training context is 2k and \texttt{rope\_base=10000}).
\end{abstract}
...
All reported numbers in this draft are produced by \path{eval_ckpt.py}, which runs (i) held-out perplexity, (ii) KV-cache bytes-per-token estimates derived from the checkpoint's \texttt{ModelConfig}, (iii) cached decode latency microbenchmarks, and (iv) a context-length sweep that tests long-context prefill and measures decode-at-context cost. The script also supports optional diagnostics (e.g., long-seq approximation drift for decoupled attention and correctness/parity checks) behind explicit flags. It writes \texttt{report.json} and CSVs into the run directory; plots are optional.

\paragraph{Training dynamics (A100; 1B; 100k steps).}
Figure~\ref{fig:a100_training_curves} summarizes the training loss, perplexity proxy, throughput (tokens/sec), and wall-clock time per step logged during the A100 runs for both \texttt{decoupled} and \texttt{baseline}.
...
\begin{subfigure}[t]{0.49\textwidth}
  ...
  \caption{Training perplexity (logged metric) vs.\ step.}
\end{subfigure}
\vspace{0.5em}
...
\begin{subfigure}[t]{0.49\textwidth}
  ...
  \IfFileExists{A100-1b-100k-tok_s.png}{
    \includegraphics[width=\textwidth]{\detokenize{A100-1b-100k-tok_s.png}}
  }{
    \fbox{\parbox{0.92\textwidth}{\textbf{Missing figure:} \texttt{A100-1b-100k-tok\_s.png}.}}
  }
  \caption{Training throughput (tokens/sec) vs.\ step.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \centering
  \IfFileExists{A100-1b-100k-ms_step.png}{
    \includegraphics[width=\textwidth]{\detokenize{A100-1b-100k-ms_step.png}}
  }{
    \fbox{\parbox{0.92\textwidth}{\textbf{Missing figure:} \texttt{A100-1b-100k-ms\_step.png}.}}
  }
  \caption{Step time (ms/step) vs.\ step.}
\end{subfigure}
\caption{A100 training curves for the 1B/100k-step runs (\texttt{baseline} vs.\ \texttt{decoupled}).}
...
For the checkpoint \path{runs/a100_fw1b_l22_decoupled_s1337_paper_strict/ckpt_step100000.pt}, \path{eval_ckpt.py} reports the following held-out language modeling quality on FineWeb-Edu token shards:
...
\IfFileExists{compare_context_decode_tok_per_sec.png}{
  ...
  Generated by \path{eval_ckpt.py} with \texttt{--context-decode} and copied into the paper directory.}}
}
...
\begin{itemize}
    \item \textbf{Baseline vs.\ decoupled (primary):} completed and reported in Section~3 (Tables~\ref{tab:ppl_compare_100k}, Figures~\ref{fig:compare_100k} and~\ref{fig:robustness_compare_100k}).
...
The bottleneck may act as an implicit regularizer by reducing the capacity of token--token interactions. In our current 1B/100k-step comparison on FineWeb-Edu shards, the standard-attention baseline attains lower perplexity (Table~\ref{tab:ppl_compare_100k}), so the primary observed benefit of DBA is efficiency rather than perplexity improvement.
...
\begin{itemize}
    \item \textbf{Quality--efficiency trade-off:} at 100k steps on FineWeb-Edu shards, the baseline achieves lower perplexity than DBA (Table~\ref{tab:ppl_compare_100k}), while DBA is faster at longer prompts and uses a substantially smaller KV cache (Figure~\ref{fig:compare_100k}).
...
We have demonstrated that attention in Transformers contains significant redundancy. In the current production-only draft, we report held-out perplexity for a decoupled 1B checkpoint trained for 100k steps (Table~\ref{tab:ppl_current}) and an apples-to-apples comparison against a standard-attention baseline trained under the same discipline (Table~\ref{tab:ppl_compare_100k}; Figures~\ref{fig:compare_100k} and~\ref{fig:robustness_compare_100k}). We also demonstrate successful long-context \textit{stability} up to 131{,}072 tokens on consumer hardware (Appendix~\ref{app:long_context_sweep}).
...
This appendix reserves space for DBA ablations (null token, tied Q--K, RoPE variants, semantic/geometric dimension splits), executed under the same manifest-driven discipline and reported via \path{eval_ckpt.py}.
