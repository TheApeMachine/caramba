...
Our implementation optionally enables a learnable per-head gate that rescales the semantic vs.\ geometric query streams before scoring. Unless explicitly labeled otherwise, all reported DBA results use the \emph{ungated} variant (\texttt{decoupled\_gate=false}) to keep the primary comparison focused on the bottleneck/decoupling itself.
...
Despite a 6\% increase in held-out perplexity, DBA exhibits no systematic capability collapse. Overall behavioral accuracy is comparable to the baseline (27.4\% vs.\ 26.5\%), with head-to-head comparisons split 7--6 in favor of DBA. At the category level, each architecture wins two categories, with eleven ties.
...
DBA exhibits degraded performance on exact copy tasks requiring precise termination. In these cases, the model frequently recalls the target content correctly but fails to stop generation at the appropriate boundary. This behavior suggests a termination or stopping signal failure rather than a content recall failure, but it remains a meaningful limitation for tasks requiring exact symbolic reproduction.
