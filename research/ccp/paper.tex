...
CCP uses an \textbf{event-native} boundary: all external interaction is a structured \texttt{EventEnvelope}. The envelope contract is:

\begin{itemize}
    \item \textbf{Required:} \texttt{type} (event type identifier), \texttt{payload} (serializable data), \texttt{sender} (stable identity).
...
For neural processing, we serialize events using \textbf{Cap'n Proto}, a zero-copy binary format with schema-defined structure. The wire format is directly memory-mappable, eliminating serialization/deserialization overhead:
\[
\mathbf{b} = \mathrm{capnp}(\mathrm{EventEnvelope}) \in \{0, \ldots, 255\}^L.
\]
...
\subsection{ResonantRouter: Phase-Coded Attractor Dynamics}
\label{subsec:resonant_router}

As an alternative to learned VQ codebooks (Section~\ref{subsec:vq_routing}), we introduce the \textbf{ResonantRouter}: a routing mechanism based on coupled-oscillator dynamics that settles to attractor states corresponding to bucket indices.

\paragraph{Motivation.} VQ routing learns discrete codebook entries via straight-through gradients, which can be unstable early in training. The ResonantRouter instead stores patterns in \emph{phase space} and uses iterative Hebbian dynamics to find the best-matching attractor. This provides smoother gradient flow during settling while still producing hard bucket indices.

\paragraph{Phase-coded patterns.} Instead of storing patterns as vectors in $\mathbb{R}^D$, we parameterize them as phases $\theta \in \mathbb{R}^{H \times B \times D}$ and project to phasors:
\[
A = \cos(\theta), \quad B = \sin(\theta).
\]
Here $H$ is the number of hashes, $B$ the number of buckets, and $D$ the routing dimension. The phasor representation $(A, B)$ lives on the unit circle in each coordinate.

\paragraph{Hebbian coupling matrix.} We construct a Hebbian weight matrix from the stored patterns. For a query phasor $(x, y) \in \mathbb{R}^D \times \mathbb{R}^D$ (representing cos and sin components), the coupling is:
\[
W_r = \frac{A^\top A + B^\top B}{D}, \quad W_i = \frac{A^\top B - B^\top A}{D}.
\]
These are the real and imaginary parts of the complex Hebbian matrix $W = \bar{P}^\top P / D$ where $P = A + iB$, but computed entirely in real arithmetic.

\paragraph{Iterative settling.} Given a query tag $q \in \mathbb{R}^D$, we initialize the state as $(x_0, y_0) = (\cos(q), \sin(q))$ and iterate:
\begin{align}
c_r &= W_r x - W_i y, \quad c_i = W_r y + W_i x, \\
x &\leftarrow \frac{(x + \gamma c_r)(1 - \zeta)}{\|(x, y)\|}, \quad
y \leftarrow \frac{(y + \gamma c_i)(1 - \zeta)}{\|(x, y)\|},
\end{align}
where $\gamma$ is the coupling strength (scaled by $1/B$ for stability), $\zeta$ is a damping factor, and the normalization projects back to the unit circle. After $S$ steps (typically 10--20), the state has settled near an attractor.

\paragraph{Bucket selection.} The final bucket index is the pattern with highest global-phase-invariant similarity:
\[
\text{sim}_k = \frac{1}{D}\sqrt{(x \cdot A_k + y \cdot B_k)^2 + (y \cdot A_k - x \cdot B_k)^2}, \quad
b = \arg\max_k \text{sim}_k.
\]
This is the magnitude of the complex inner product $|\langle (x + iy), (A_k + iB_k) \rangle|$, computed in real arithmetic.

\paragraph{Properties.}
\begin{itemize}
    \item \textbf{Constant-time:} Each iteration is $O(D^2)$ for the matmul; with small $D$ (e.g., 64--256) and fixed $S$, this is effectively $O(1)$ relative to sequence length.
    \item \textbf{Smooth gradients:} The iterative dynamics provide gradient signal at every step, unlike hard VQ assignments.
    \item \textbf{Attractor stability:} Patterns act as fixed points; strong patterns attract nearby queries.
    \item \textbf{Real-valued implementation:} The $(x, y)$ representation avoids complex-number operations, improving numerical stability on some hardware.
\end{itemize}

The ResonantRouter is enabled via \texttt{mem\_router: resonant} in the manifest.
...
\begin{itemize}
    ...
    \item \textbf{mosaic\_icl\_rule\_induction\_resonant.yml}: ICL with ResonantRouter (phase-coded routing).
\end{itemize}
...
\subsection{ResonantRouter ICL (Diagnostic)}

We evaluated the ResonantRouter (Section~\ref{subsec:resonant_router}) on the same ICL rule induction task. The ResonantRouter replaces VQ codebook routing with phase-coded attractor dynamics.

\paragraph{Setup.} The model uses $H=2$ hashes, $B=256$ buckets, 20 settling steps, coupling $\gamma=0.25$ (scaled by $1/B$), and damping $\zeta=0.02$. Training uses \texttt{float32} precision for numerical stability during the iterative dynamics.

\paragraph{Results.} At step 298, training loss reached $\sim$3.75 (PPL $\sim$42) with no NaN/Inf events, demonstrating that the phase-coded dynamics are numerically stable. The loss curve shows consistent descent from $\sim$6.0 to $\sim$3.5 over 300 steps, with healthy throughput ($\sim$500--2500 tok/s) and sub-20ms step times. Memory telemetry confirms active read gates.

\paragraph{Observations.}
\begin{itemize}
    \item The iterative settling provides smoother optimization dynamics compared to straight-through VQ gradients.
    \item The real-valued $(x, y)$ representation (avoiding complex-number operations) improves hardware compatibility.
    \item Per-bin accuracy remains at 0\%, consistent with the VQ baseline---the addressing bottleneck persists regardless of routing mechanism.
\end{itemize}

This confirms that the ResonantRouter is a viable alternative routing mechanism, but does not by itself solve the addressing problem. The value of phase-coded routing may emerge in longer training runs or with complementary curriculum design.
...
\begin{itemize}
    ...
    \item \path{config/presets/mosaic_icl_rule_induction_resonant.yml} --- ICL with ResonantRouter.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Routing:} \path{layer/memory_block/memory/routing.py} --- BitRouter, VqRouter, ResonantRouter.
