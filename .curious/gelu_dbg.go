...
package main
import ("fmt"; "math")
func erfSim(value float32) float32 {
	...
	if absValue < point84375 {
		...
		fmt.Printf("small region ratio=%g temp=%g\n", ratio, temp)
		if sign { return -temp }; return temp
	}
	if absValue < point25 {
		shift := absValue - one
...
func main() {
	t := float32(-0.8249579)
	fmt.Println("erf sim", erfSim(t), "go", float32(math.Erf(float64(t))))
	x := float32(-1.1666667)
	g := 0.5*x*(1+erfSim(x*0.7071067811865475))
	fmt.Println("gelu", g)
}
