...
import type { CommentAction } from "../../commentsReducer";
...
import type { StageActionSetter } from "../../stageReducer";
import { StageActionType } from "../../stageReducer";
...
const Stage = ({
    ...
    const handleWheel = React.useCallback(
        (e: WheelEvent) => {
            ...
            if (
                wheelTarget.nodeName === "TEXTAREA" ||
                wheelTarget.dataset.comment
            ) {
    ...
    const addNode = ({ node, internalType }: SelectOption) => {
        ...
        if (wrapperRect) {
            const x =
                byScale(
                    menuCoordinates.x - wrapperRect.x - wrapperRect.width / 2
                ) + byScale(translate.x);
            const y =
                byScale(
                    menuCoordinates.y - wrapperRect.y - wrapperRect.height / 2
                ) + byScale(translate.y);
