...
const reconcileNodes = (
    ...
    let reconciledNodes = Object.values(nodes).reduce<NodeMap>(
        (nodesObj, node) => {
            ...
            const currentInputData = Object.entries(
                node.inputData
            ).reduce<InputData>((dataObj, [key, data]) => {
...
export type NodesAction =
    | {
          type:
              | NodesActionType.ADD_CONNECTION
              | NodesActionType.REMOVE_CONNECTION;
          input: ProposedConnection;
          output: ProposedConnection;
      }
    | {
    ...
    switch (action.type) {
        case NodesActionType.ADD_CONNECTION: {
            ...
            if (inputIsNotConnected) {
                ...
                const isCircular = checkForCircularNodes(
                    newNodes,
                    output.nodeId
                );
