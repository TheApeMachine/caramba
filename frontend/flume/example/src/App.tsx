...
flumeConfig
    ...
    .addNodeType({
        ...
        outputs: (ports) => [
            ports.text({ name: "result", label: "Joined Text" })
        ]
    })
    .addNodeType({
        type: "textSwitch",
        label: "Text Switch",
        description:
            "Outputs one string if true, or a different string if false",
        initialWidth: 180,
        inputs: (ports) => [
...
const Website = ({ nodes }: { nodes: NodeMap }) => {
    const { title, description, showDashboard, showBody, showLogin } =
        useInfiniteEngine<{
            title: string;
            description: string;
