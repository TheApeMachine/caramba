...
export const Route = createFileRoute("/api/shape/assistant-sessions")({
	server: {
		handlers: {
			GET: async ({ request }) => {
				...
				upstream.searchParams.set("table", "assistant_sessions");
				upstream.searchParams.set(
					"where",
					`(scope='team' AND organization_slug=$1) OR (scope='personal' AND owner_id=$2)`,
				);
