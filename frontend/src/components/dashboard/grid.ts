...
export class Grid {
	...
	private occupants(
		...
		for (let deltaCol = 0; deltaCol < colSpan; deltaCol++) {
			for (let deltaRow = 0; deltaRow < rowSpan; deltaRow++) {
				const found = this.get(col + deltaCol, row + deltaRow);
...
/*
BentoTileSpec is input to packBentoLayout: desired footprint and placement
priority. Larger priority tiles are placed first; ties break on area.
*/
...
const markFootprint = (
	...
	for (let deltaRow = 0; deltaRow < rowSpan; deltaRow++) {
		for (let deltaCol = 0; deltaCol < colSpan; deltaCol++) {
			occupied[row + deltaRow][col + deltaCol] = true;
		}
	}
};
...
const tryPlaceTile = (
	...
	const spans: Array<{ colSpan: number; rowSpan: number }> = [
	...
	for (const span of spans) {
		for (let row = 0; row < rows; row++) {
			for (let col = 0; col < cols; col++) {
				...
				markFootprint(occupied, col, row, span.colSpan, span.rowSpan);
...
/*
packBentoLayout greedily packs tiles into a fixed GRID_COLS x GRID_ROWS
canvas. Tiles are ordered by priority then area so prominent widgets
claim space first; anything that does not fit is omitted.
*/
export const packBentoLayout = (
	tiles: ReadonlyArray<BentoTileSpec>,
	...
	const sorted = [...tiles].sort((left, right) => {
		...
		return right.colSpan * right.rowSpan - left.colSpan * left.rowSpan;
	});

	const placed: PlacedWidget[] = [];

	for (const tile of sorted) {
		const widget = tryPlaceTile(occupied, tile, cols, rows);
