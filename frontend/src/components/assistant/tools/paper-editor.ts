...
export const paperInsertBlock = toolDefinition({
	...
	description:
		"Inserts a new block (paragraph, heading, equation, or list) after the given block ID. Use afterId 'last' to append at end.",
	inputSchema: {
		type: "object" as const,
		...
		properties: {
			...
			blockType: {
				type: "string",
				enum: ["paragraph", "heading", "equation", "list"],
			},
			text: {
				type: "string",
				description:
					"Text for paragraph or heading; newline-separated items for list.",
			},
			level: {
				type: "number",
				enum: [1, 2, 3],
				description: "Heading level (1–3). Required for heading blocks.",
			},
			latex: {
				type: "string",
				description: "LaTeX source for equation blocks.",
			},
			ordered: {
				type: "boolean",
	...
	const typed = args as {
		afterId: string;
		blockType: "paragraph" | "heading" | "equation" | "list";
		text?: string;
		level?: 1 | 2 | 3;
		latex?: string;
		ordered?: boolean;
	...
	} else if (typed.blockType === "list") {
		newId = api.insertListAfter(afterId, typed.ordered ?? false);
		if (typed.text) api.updateText(newId, typed.text);
	} else {
...
export const paperUpdateBlock = toolDefinition({
	...
	inputSchema: {
		...
		properties: {
			...
			text: {
				type: "string",
				description: "New text for paragraph or heading blocks.",
			},
