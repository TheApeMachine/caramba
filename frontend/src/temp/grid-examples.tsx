...
export const GridResponsiveExample = () => {
	...
	return (
		<div className="space-y-4">
			<div>
				<h3 className="text-lg font-semibold mb-2">Grid.Responsive (cols=6)</h3>
				<p className="text-sm text-muted-foreground mb-4">
					Responsive breakpoints: 1 col mobile → 2 col sm → 3 col md → 4 col lg
					→ 6 col xl
				</p>
			</div>
			<Grid cols={6} gap={4}>
				{items.map((item) => (
					<ExampleCard key={item.id} title={item.title} number={item.id} />
				))}
			</Grid>
		</div>
	);
	...
	return (
		<div className="space-y-4">
			<div>
				...
				<p className="text-sm text-muted-foreground mb-4">
					Automatically fits items based on container width, minimum 250px per
					item
				</p>
			</div>
	...
	return (
		<div className="space-y-8">
			<div>
				...
				<p className="text-sm text-muted-foreground mb-4">
					Layout automatically optimizes based on the exact number of items
				</p>
			</div>
				...
				<div key={count} className="space-y-2">
					...
					<Grid.Adaptive gap={4}>
						{Array.from({ length: count }, (_, i) => {
							const id = `${count}-${i + 1}`;
							return <ExampleCard key={id} title="Card" number={i + 1} />;
						})}
					</Grid.Adaptive>
				</div>
			))}
