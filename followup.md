...
- **Markov blanket**: `markov_blanket.metal` — partition (device), flow internal/active (parallel rows), Gaussian MI (fully parallelized covariance computation + device Cholesky / log-det in float).
- **Causal**: `causal.metal` — parallel axpy/sub/matvec, atomic dot reduction. Do-calculus, backdoor, IV, CATE, DAG–Markov factorisation now use **fully parallelized linear algebra pipelines** (parallel `X^T X`, `X^T Y`, and matrix multiplications) orchestrated from Objective-C, eliminating the single-threaded bottlenecks for large `T` datasets.
